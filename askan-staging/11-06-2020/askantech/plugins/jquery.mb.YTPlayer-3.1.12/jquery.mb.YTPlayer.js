  ...
  jQuery.mbYTPlayer = {
    ...
    buildPlayer: function (options) {
      ...
      return this.each(function () {
        var YTPlayer = this;
        var jqueryYTPlayer = jQuery(YTPlayer);
        jqueryYTPlayer.hide();
        YTPlayer.loop = 0;
        YTPlayer.state = 0;
        ...
        YTPlayer.opt = jQuery.extend({}, options);
	      /* Set properties */
	      YTPlayer.property = jqueryYTPlayer.data("property") && typeof jqueryYTPlayer.data("property") == "string" ?
        ...
        if (typeof YTPlayer.property != "undefined") {
          ...
          /* Manage containment */
          YTPlayer.property.containment = YTPlayer.property.containment == "self" ? jqueryYTPlayer : jQuery(YTPlayer.property.containment);

        }

        YTPlayer.isRetina = ( window.retina || window.devicePixelRatio > 1 );

        if (!jqueryYTPlayer.attr("id"))
          jqueryYTPlayer.attr("id", "ytp_" + new Date().getTime());
        ...
        YTPlayer.videoID = YTPlayer.property.videoURL ?
            getYTPVideoID(YTPlayer.property.videoURL).videoID : jqueryYTPlayer.attr("href") ?
                getYTPVideoID(jqueryYTPlayer.attr("href")).videoID :
                false;

        /* Check if it is a video list */
        YTPlayer.playlistID = YTPlayer.property.videoURL ?
            getYTPVideoID(YTPlayer.property.videoURL).playlistID : jqueryYTPlayer.attr("href") ?
        ...
        YTPlayer.isPlayer = YTPlayer.opt.containment == "self" || (jqueryYTPlayer.is (YTPlayer.opt.containment));
        ...
        jQuery(document).one("YTAPIReady", function () {
          jqueryYTPlayer.trigger("YTAPIReady_" + YTPlayer.id);
          ytp.YTAPIReady = true;
        });

        jqueryYTPlayer.one("YTAPIReady_"+YTPlayer.id, function(){
          ...
          jQuery(YTPlayer).on("YTPChanged", function (e) {
            ...
            new YT.Player(YTPlayer.playerID, {
              ...
              events: {
                'onReady': function (event) {
                  ...
                  jqueryYTPlayer.trigger("YTPlayerIsReady_" + YTPlayer.id);
                },
                /**
                 * on State Change
                 * @param event
                ...
                'onError': function (err) {
                  ...
                  if (typeof YTPlayer.opt.onError == "function")
                    YTPlayer.opt.onError(jqueryYTPlayer, err);
            ...
            jqueryYTPlayer.on("YTPlayerIsReady_" + YTPlayer.id, function(){
              ...
              jqueryYTPlayer.YTPCheckForState();

            });
          });
        });

        jqueryYTPlayer.off("YTPTime.mask");
    ...
    playlist: function (videos, shuffle, callback) {
      ...
      var jqueryYTPlayer = this;
      var YTPlayer = jqueryYTPlayer.get(0);
        ...
        function getVideoListFromYoutube(playListID, page) {
          ...
          jQuery.getJSON(youtubeAPI, {
            ...
            if (response.nextPageToken) {
              ...
              jqueryYTPlayer.YTPlaylist(YTPlayer.videos, shuffle, callback)
            };
          });
        };
    ...
    changeVideo: function (opt) {
      ...
      jQuery(YTPlayer.playerEl).CSSAnimate({
        opacity: 0
	      ...
	      jqueryYTPlayer.YTPGetPlayer().loadVideoById({
          videoId: YTPlayer.videoID,
          // startSeconds: YTPlayer.opt.startAt,
          // endSeconds: YTPlayer.opt.stopAt,
          suggestedQuality: YTPlayer.opt.quality
        });
	      jqueryYTPlayer.YTPPause();
	      jqueryYTPlayer.optimizeDisplay();
    ...
    applyFilter: function (filter, value) {
      ...
      if (YTPlayer.filtersEnabled)
        jqueryYTPlayer.YTPEnableFilters();
    },
    ...
    applyFilters: function (filters) {
      ...
      if (!YTPlayer.isReady) {
        jQuery(YTPlayer).on("YTPReady", function () {
          jqueryYTPlayer.YTPApplyFilters(filters);
        });
        return this;
      }

      for (var key in filters) {
        jqueryYTPlayer.YTPApplyFilter(key, filters[key]);
      }

      jqueryYTPlayer.trigger("YTPFiltersApplied");
    },
    ...
    removeFilter: function (filter, callback) {
      ...
      if (!filter) {
        for (var key in YTPlayer.filters) {
          jqueryYTPlayer.YTPApplyFilter(key, 0);
        }
        ...
        jqueryYTPlayer.YTPApplyFilter(filter, 0);
        if (typeof callback == "function") callback(filter);
      }

      var YTPEvent = jQuery.Event("YTPFiltersApplied");
      jqueryYTPlayer.trigger(YTPEvent);
    ...
    applyMask: function (YTPlayer) {
      ...
      if (YTPlayer.opt.mask) {
        if (typeof YTPlayer.opt.mask == "string") {
          jqueryYTPlayer.YTPAddMask(YTPlayer.opt.mask);
          ...
          if (YTPlayer.opt.mask[0])
            jqueryYTPlayer.YTPAddMask(YTPlayer.opt.mask[0]);
          jqueryYTPlayer.on("YTPTime.mask", function (e) {
            for (var time in YTPlayer.opt.mask) {
              if (e.time == time)
                if (!YTPlayer.opt.mask[time]) {
                  jqueryYTPlayer.YTPRemoveMask();
                } else {
                  jqueryYTPlayer.YTPAddMask(YTPlayer.opt.mask[time]);
    ...
    checkForState: function () {
      ...
      if (!jQuery.contains(document, YTPlayer)) {
        jqueryYTPlayer.YTPPlayerDestroy();
        clearInterval(YTPlayer.getState);
        clearInterval(YTPlayer.checkForStartAt);
        return;
      }
      ...
      YTPlayer.getState = setInterval(function () {
        ...
        if (YTPlayer.player.getVolume() == 0) jqueryYTPlayer.addClass("isMuted");
        else jqueryYTPlayer.removeClass("isMuted");
    ...
    checkForStart: function (YTPlayer) {
      ...
      YTPlayer.checkForStartAt = setInterval(function () {
        ...
        if (YTPlayer.player.getDuration() > 0 && YTPlayer.player.getCurrentTime() >= startAt && canPlayVideo) {
          ...
          jqueryYTPlayer.YTPRemoveFilter();

          if (YTPlayer.opt.addFilters) {
            jqueryYTPlayer.YTPApplyFilters(YTPlayer.opt.addFilters);
          } else {
            jqueryYTPlayer.YTPApplyFilters({});
          }
          jqueryYTPlayer.YTPEnableFilters();
    ...
    setAnchor: function (anchor) {
      var jqueryYTplayer = this;
      jqueryYTplayer.optimizeDisplay(anchor);
    },
