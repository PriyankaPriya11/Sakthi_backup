...
class SR_WC_P360V {
    ...
    private static $sr_plugin_version = '3.1.1';
    ...
    function __construct() {
        if (in_array('woocommerce/woocommerce.php', apply_filters('active_plugins', sanitize_option('active_plugins', get_option('active_plugins'))))) {
            ...
            add_action('wp_enqueue_scripts', array('SR_WC_P360V', 'sr_register_scripts'));
            add_action('wp_enqueue_scripts', array('SR_WC_P360V', 'sr_enqueue_scripts'));
    ...
    static function sr_wc_p360v_update_setting() {
        ...
        if (check_ajax_referer('sr-wc-p360v-settings-page', 'security')) {
            ...
            $settings['sr_wc_p360v_image_rotation_speed'] = intval($_POST['sr_wc_p360v_image_rotation_speed']);
            if (!in_array($settings['sr_wc_p360v_image_rotation_speed'], array(20, 40, 60, 80))) {
            ...
            $settings['sr_wc_p360v_image_size'] = sanitize_text_field($_POST['sr_wc_p360v_image_size']);
            if ($settings['sr_wc_p360v_image_size'] !== 'original' && $settings['sr_wc_p360v_image_size'] !== 'stretch'):
                $response = array(
                    'error' => __('Wrong input for image size.', 'sr-product-360-view')
                );
                echo json_encode($response);
                exit;
            endif;
    ...
    static function sr_register_scripts() {
        ...
        wp_register_script('threesixty', plugins_url('/assets/js/front-end/threesixty.min.js', __FILE__), array('sr-wc-360'));
    }
    ...
    static function sr_add_plugin_options() {
        ...
        add_option('sr_wc_p360v_image_rotation_control_1', __('360,move,drag', 'sr-product-360-view'));
        ...
        add_option('sr_wc_p360v_image_size', __('original', 'sr-product-360-view'));
