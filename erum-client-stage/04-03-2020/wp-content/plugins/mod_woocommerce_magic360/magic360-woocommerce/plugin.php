...
function WooCommerce_Magic360_send_stat($action = '') {
    ...
    if('working' == 'v6.8.12' || 'working' == 'v4.6.10') {
        return;
    }
    ...
    $path = "api/stat/?action={$action}&tool_name=magic360&license=trial&tool_version=v4.6.10&module_version=v6.8.12&platform_name=woocommerce&platform_version={$platformVersion}&url={$url}";
...
function  magictoolbox_WooCommerce_Magic360_init() {
    ...
    $allParams['product']['selector-path']['default'] = preg_replace('/.*(\/wp-content.*)/','$1', plugins_url('/core/360icon.jpg', __FILE__));

    delete_option("WooCommerceMagic360CoreSettings");
        add_option("WooCommerceMagic360CoreSettings", $allParams);
    }
...
function WooCommerce_Magic360_import() {
    ...
    if (array_key_exists('tool', $arr) && $tool == $arr['tool']) {
        ...
        if (array_key_exists('core', $arr)) {
            ...
            delete_option("WooCommerceMagic360CoreSettings");
            add_option("WooCommerceMagic360CoreSettings", $settings);
        }
...
function WooCommerce_Magic360_get_product_variations ($product_id = false) {
    ...
    if ( $product->is_type('variable')) {
        ...
        if (is_array($variations) && count($variations) > 0) {
            ...
            $a = str_replace('<a','<a onclick="onMagic360SelectorClick(this);" ',$a);
...
function WooCommerce_Magic360_locate_template( $template, $template_name, $template_path ) {
  ...
  $template = locate_template(
    ...
    if ( metadata_exists( 'post', $post_id, '_magic360_data' )) {
        ...
        if (!empty($magic360_data) && array_key_exists('images_ids', $magic360_data) && count($magic360_data['images_ids']) && file_exists( $plugin_path . $template_name ) ) {
    ...
    if (metadata_exists( 'post', $post_id, '_magic360_data' ) && magictoolbox_WooCommerce_Magic360_check_plugin_active('magic360') ){
        ...
        if(!empty($magic360_plugin) && $magic360_plugin->params->checkValue('page-status','Yes') ){
            ...
            if(!empty($magic360_image_gallery)){
                ...
                if (!$useWpImages) {
                    $magic360_selector = '<a data-product-id="'.$post_id.'" data-magic-slide-id="360" style="display:inline-block;" class="m360-selector" title="360" href="#" onclick="return false;"><img src="'.WooCommerce_Magic360_get_product_image($magic360_selector_path,'selector').'" alt="360" /></a>';
                } else {
