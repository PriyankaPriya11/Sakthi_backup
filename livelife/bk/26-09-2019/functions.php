 ...
 'article-menu1' => __('article-menu1'),
  ...
  function wpb_widgets_init() {
    ...
    if(!is_admin()) {
        ...
        // $handle;
        if ( 'jmin-js' == $handle){ return $tag; }
        if ( 'owl.carousel.js' == $handle){ return $tag; }
        if ( 'swipe-nav' == $handle){ return $tag; }
 ...
 */
// include('protect/slug.php');
function orbisius_ct_gridlove_child_theme_child_theme_enqueue_styles() {
    $parent_style = 'orbisius_ct_gridlove_child_theme_parent_style';
    $parent_base_dir = 'gridlove';
...
add_action( 'wp_enqueue_scripts', 'orbisius_ct_gridlove_child_theme_child_theme_enqueue_styles' );
...
function footer_menu() {
    register_sidebar( array(
        ...
        'id' => 'footer_menu',
        'description' => __( 'Footer Menu', 'wpb' ),
        'before_widget' => '<div id="%1$s" class="widget %2$s box cf link-black col-lg-half">',
		'after_widget'  => '</div>',
...
function html5_search_form( $form ) { 
     /*$form = '<form role="search" method="get" id="searchform" class="gridlove-search-form" action="' . esc_url( home_url( '/' ) ) . '">
				<div>
					<label class="screen-reader-text" for="s">' . _x( 'Search for:', 'label' ) . '</label>
					<input type="text" value="' . get_search_query() . '" name="s" id="s" placeholder="' . esc_attr_x( 'Search', 'placeholder' ) . '"/>
				</div>
            </form>'; if(isset($_REQUEST['swpquery'])){ 
            $word = $_REQUEST['swpquery'];*/
            $word = "";
            if(isset($_REQUEST['swpquery'])){ $word = $_REQUEST['swpquery']; $word = str_replace("+"," ",$word); }else{ $word = 'Search Guides...'; }
            $id = get_field( 'fid', 8214);
            //if(get_field('fid', 8214)){$id = get_field( 'fid', 8214)}
        $form = '<form role="search" method="get" id="searchform" class="gridlove-search-form" action="'.site_url().'/search2.php'.'">
                    <label class="screen-reader-text" for="s">' . _x( 'Search for:', 'label' ) . '</label>
                        <input class="search_field" id="search_field" type="text" placeholder="Search Guides..." value="'.$word.'" name="swpquery" title="Search for:">
...
function related_search_form( $form ) { 
           $word = "";
           if(isset($_REQUEST['swpquery'])){ $word = $_REQUEST['swpquery']; $word = str_replace("+"," ",$word); }else{ $word = 'Search Guides...'; }
           $id = get_field( 'fid_related', 8294);
           //if(get_field('fid', 8214)){$id = get_field( 'fid', 8214)}
       $form = '<form role="search" method="get" id="searchform" class="gridlove-search-form" action="'.site_url().'/search2.php'.'">
                    <label class="screen-reader-text" for="s">' . _x( 'Search for:', 'label' ) . '</label>
                        <input class="search_field" id="search_field" type="text" placeholder="Search Guides..." value="'.$word.'" name="swpquery" title="Search for:">
...
function highlight_script() {
    if ((is_search()) || (is_page(8214)) ){
        wp_enqueue_script(
            'highlight-script',
            get_stylesheet_directory_uri() . '/js/jQuery.highlight.js',
            array( 'jquery' )
        );
    }
}

add_action( 'wp_enqueue_scripts', 'highlight_script' );

function wpb_adding_scripts() {
    wp_register_script('jmin-js', get_stylesheet_directory_uri().'/_css/jquery.min.js', array('jquery'),'1.1', false);
    wp_enqueue_script('jmin-js');
    wp_register_script('owl.carousel.js', get_stylesheet_directory_uri().'/_css/owl.carousel.js', array('jmin-js'),'', false);
    wp_enqueue_script('owl.carousel.js');
    wp_register_script('swipe-nav', get_stylesheet_directory_uri().'/js/jquery-swipe-nav-plugin.js', array('owl.carousel.js'),'', false);
    wp_enqueue_script('swipe-nav');
}
...
{
    wp_dequeue_script('gridlove-main'); wp_deregister_script('gridlove-main');
}
add_action( 'wp_head', 'my_deregister_styles', 1 );
add_action( 'wp_print_styles','my_deregister_styles', 1 );
function my_deregister_styles() {
        wp_deregister_style('gridlove-main-css');
}
...
/* get infospace signature */
function infospace_getsignature() {
    $queryTerm = $_REQUEST['swpquery'];
    $token = 'uuB7HH0q6EO_SUK5gfZDhQ2';
    $signer = new InfoSpaceRequestSigner($token);
    return $signer->getSignature($queryTerm);
}
...
/*function searchwp_logic(){
    if(is_page(8214)){
        return '__return_true';
    }
}
add_filter( 'searchwp_and_logic_only', 'searchwp_logic' );*/

add_action( 'searchwp_before_query_index', function( $args ){
	$engine_or = array( 'related_key_search' );
	if( in_array( $args['engine'], $engine_or ) ){
...
class swp_add_synonyms_to_content{
	public $synonyms;
	function __construct(){
		$this->synonyms = get_option( 'swp_termsyn_settings' );
		if( ! empty( $this->synonyms ) ){
			add_filter( 'searchwp_indexer_pre_process_content', array( $this, 'add_synonyms_to_content' ) );
		}
	}
	public function add_synonyms_to_content( $content ){
		$synonyms_to_add = '';
		if( '' !== $content ){
			//Add synonyms if a term is present in the content
			foreach( $this->synonyms as $synonym ){
				//If the term is in the content we get a list of synonyms to add to it
				if( 1 == preg_match( '/\b' . $synonym['term'] . '\b/i', $content ) ){
					$current_synonyms = explode( ' ', $synonym['synonyms'][0] );
					//If the synonym is already present in the content we won't add it to avoid redundancy
					foreach( $current_synonyms as $current_synonym ){
						if( 0 == preg_match( '/\b' . $current_synonym . '\b/i', $content ) ){
							$synonyms_to_add .= ' ' . $current_synonym;
						}
					}
				}
				//Add a term if a synonym is present in the content
				//This works on reverse of the previous function
				if( 1 == preg_match( '/\b' . $synonym['synonyms'][0] . '\b/i', $content ) ){
					$current_synonym = $synonym['term'];
					//If the synonym is already present in the content we won't add it to avoid redundancy
					if( 0 == preg_match( '/\b' . $current_synonym . '\b/i', $content ) ){
						$synonyms_to_add .= ' ' . $current_synonym;
					}
			...
			if( $synonyms_to_add ){
				...
				$synonyms_to_add = explode( ' ', $synonyms_to_add );
				$synonyms_to_add = implode( ' ', array_unique( $synonyms_to_add ) );
				//Add the synonyms to the content
				$content = $content . ' ' . $synonyms_to_add;
			}
		}
		return $content;
	}
}
new swp_add_synonyms_to_content();
...
function my_modify_title( $title, $id ) {
    if( in_category( 'Breeds of Dog' , $id ) ) {
        if (strpos($title, 'Dog Breed Information and Pictures') === false) {
            $title = $title . ' – Dog Breed Information and Pictures';
            $my_post = array(
                'ID'           => $id,
                'post_title'   => $title,
            );
...
if( function_exists('acf_add_options_page') ) {
	...
	acf_add_options_page('FID Template Change');
