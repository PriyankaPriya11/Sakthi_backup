                                ...
                                <div class="tab-pane active" id="description">
                                      ...
                                      <?php          
                                                  echo   '    <div class="notice notice-info">
                                                                <p>Upgrade to the pro version now using coupon code JCHGOPRO20 for a 20% discount!!</p> </div> ';        ?>
                                                ...
                                                <div class="notice notice-info">
                                                        ...
                                                        <p><?php printf(wp_kses(__('If you use this plugin please consider posting a review on the plugin\'s <a href="%s" target="_blank" >WordPress page</a>. If you\'re having problems, please submit for support and give us a chance to resolve your issues before reviewing. Thanks.', 'jch-optimize'), array(
                                                                        'a' => array(
                                                                                'href'   => array(),
                                                                                'target' => array()
                                                                        )
...
function jch_initialize_settings()
{
        ...
        add_settings_section('jch_remove_js', '', 'jch_remove_js_section_text', 'jch-sections');
        add_settings_field('jch_options_remove_js', __('Remove javascript', 'jch-optimize'), 'jch_options_remove_js_string', 'jch-sections', 'jch_remove_js');

        add_settings_section('jch_pro_reduce_dom', '', 'jch_pro_reduce_dom_section_text', 'jch-sections');
        add_settings_field('jch_options_pro_reduce_dom', __('Enable', 'jch-optimize'), 'jch_options_pro_reduce_dom_string', 'jch-sections', 'jch_pro_reduce_dom');
...
function jch_remove_js_section_text()
{
        jch_group_end();

        $header = __('Remove unused javascript', 'jch-optimize');
        $description = __('Identify and select the javascript files that are not being used on the site. The plugin will remove it from the page and prevent it from loading.', 'jch-optimize');
        jch_group_start($header, $description);
}

function jch_options_remove_js_string()
{
        $option = 'remove_js';
        $values = jch_get_field_value('js', $option, 'file');

        echo jch_gen_multiselect_field($option, $values, '');
}

function jch_pro_reduce_dom_section_text()
{
        jch_group_end();

        $header = __('Reduce DOM size', 'jch-optimize');
        $description = __('<span class="label label-important" style="padding: 1px 3px; ">Beta feature!</span> HTML5 sectioning related elements such as header, footer, section, aside, nav, and article that fall below the fold will be commented out if the number of DOM elements exceed 600. These sections will be inserted back into the DOM using javascript after the page has been loaded. <span class="label label-warning" style="padding: 1px 3px; ">Warning: Use with care. Can interfere with sections that use javascript to render.</span>', 'jch-optimize');

        jch_group_start($header, $description);
}

function jch_options_pro_reduce_dom_string()
{
